  ...
  const renderConnectedContainer = () => {
    ...
    if (articleList === null) {
      ...
      {articleList.map((items, index) => (
        <ArticleCard key={index} articleId={items["articleId"]} walletAdd={items["walletAdd"]} content={items["content"]} articleTags={articleTags[articleTags.findIndex((item, i) => {return item.id === items["articleId"]})]}/>
      ))}
      </div>
      )
    }
  ...
  return (
    <div className="flex">
      ...
      <div className="bg-slate-900 h-screen w-screen grid place-items-center">
        <Image src="/solana-sol-logo.png" width={200} height={200} />
        <Select
                className='w-1/2'
